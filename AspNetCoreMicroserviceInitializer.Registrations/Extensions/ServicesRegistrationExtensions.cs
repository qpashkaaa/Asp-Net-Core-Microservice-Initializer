...
internal static class ServicesRegistrationExtensions
{
    ...
    {
        var servicesInfo = GetOrderedServicesTypes(services);
    ...
    private static IOrderedEnumerable<ServiceTypeInfo> GetOrderedServicesTypes(IServiceCollection services)
    {
        var servicesInfo = new List<ServiceTypeInfo>();

        var serviceProvider = services.BuildServiceProvider();

        var assemblies = AssemblyHelper.LoadAssembliesWithSpecificAttribute<AutoRegisterServiceAttribute>(false, serviceProvider).ToList();

        AssemblyHelper.FindTypesByConditionAndDoActions(
            assemblies,
            assembly => assembly.GetTypes().Where(t => t.GetCustomAttributes<AutoRegisterServiceAttribute>(false).Any()),
            type =>
            {
                var attribute = type.GetCustomAttribute<AutoRegisterServiceAttribute>() ?? throw new Exception($"У модели {type.Name} не указан атрибут {nameof(AutoRegisterServiceAttribute)} для автоматической регистрации сервиса.");
                ...
                var serviceInfo = new ServiceTypeInfo
                {
                    ...
                    ImplementationFactory = GetImplementationFactory(type)
                };
    ...
    /// </returns>
    private static Func<IServiceProvider, object>? GetImplementationFactory(Type type)
    {
